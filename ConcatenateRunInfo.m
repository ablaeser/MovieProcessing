...
expt.sbx.cat = info.path;
regPath = info.path;
regPath(end-2:end) = 'reg';
if exist(regPath, 'file')
    expt.sbx.reg = regPath;
else
    expt.sbx.reg = '';
end
...
%{
if expt.Nplane > 1
    expt.sbx = strcat(expt.dir, expt.name, 'sbxcat' ); % '.sbx_interp'
else
    expt.sbx = strcat(expt.dir, expt.name, '.sbxreg');
end
%}
end