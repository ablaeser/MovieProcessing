...
if nargin > 3  %~exist(projParamPath, 'file') % || projParam.overwrite
    ...
    for t = 1:projParam.Ntype
        if exist(expt.sbx.(projParam.sbx_type{t}), 'file')
            ...
            temp_vol = cell(expt.Nruns, 1);  tempRunProj = cell(expt.Nruns, 2, projParam.Nz);  runBinLims = cell(1,expt.Nruns);
            runProjCell = cell(expt.Nruns, 3, projParam.Nz); runVolCell = cell(expt.Nruns, 3); catCell = cell(projParam.Nz, 3);
            projParam.path.run.(projParam.sbx_type{t}).z = runProjCell;  projParam.path.run.(projParam.sbx_type{t}).vol = runVolCell;
            projParam.path.cat.(projParam.sbx_type{t}).z = runProjCell;  projParam.path.cat.(projParam.sbx_type{t}).vol = cell(1,3);
            for runs = 1:expt.Nruns
                ...
                if expt.Nplane == 1
                    [~, tempRunProj(runs,:,1), runBinLims{runs}, tempRawProjPath] = WriteSbxPlaneTif(expt.sbx.(projParam.sbx_type{t}), catInfo, 1, 'chan','both', 'firstScan',expt.scanLims(runs)+projParam.bin+1, 'Nscan',expt.Nscan(runs)-projParam.bin, ...
                        'edge',projParam.edge, 'scale',projParam.scaleFactor, 'binT',projParam.bin, 'RGB',false, 'dir',runsDir, 'name',tempName, 'sbxType',projParam.sbx_type{t}, 'overwrite',projParam.overwrite );
