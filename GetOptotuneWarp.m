function tforms_optotune = GetOptotuneWarp(sbxPath, sbxInfo, varargin)
IP = inputParser;
...
addParameter(IP, 'type', 'affine', @ischar); %options are 'affine', 'rigid', or 'none'
...
tformPath = sprintf('%stforms_optotune.mat',fovDir); %'D:\2photon\CGRPAi6-1\220816_FOV1\tforms_optotune.mat'; % strcat(fdir,filesep,'tforms_optotune.mat');
if ~exist(tformPath, 'file')
    ...
    % Test the results
    %{
saveastiff(uint16(ref_vol), 'D:\2photon\CGRP02\201110_CGRP02\201110_CGRP02_run7\refVol.tif' );
for z = flip(1:sbxInfo.Nplane)
    reg_vol(:,:,z) = imwarp( ref_vol(:,:,z), tforms_optotune(z), 'OutputView',imref2d(size(ref_vol,[1,2])));    %#ok<AGROW>
end
saveastiff(uint16(reg_vol), 'D:\2photon\CGRP02\201110_CGRP02\201110_CGRP02_run7\regVol.tif' );
    %}
    %save the transformations for later

    if saveToggle
        fprintf('\nSaving %s', tformPath)
        save(tformPath,'tforms_optotune'); %strcat(fdir,filesep,'tforms_optotune.mat')
    end
else
    fprintf('\nLoading %s', tformPath)
