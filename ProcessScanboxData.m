...
regParam.edges = [100,100,40,40]; % cut this many pixels off from the [left, right, top, bottom] to avoid edges contaminating the registration
...
xPresent = 109; %44; %[18,22,24,30:32]; % flip(100:102); %45:47; % [66:69]; %6;  62,64,
...
for x = xPresent  %30 %x2D % x2Dcsd % x3D %% 51
    ...
    interRunShift = ConcatenateExptRuns(expt{x}, runInfo{x}, catInfo(x), 'refRun',regParam.refRun, 'refChan',regParam.refChan, 'setEdge',regParam.edges); %1  expt{x}.refChan
    ...
    projParam.edge = [60,60,40,20]; % [40,40,40,80];%segParams{x}.edges; %[60,60,20,20]; % [70 40 20 20];
    projParam.z =  {17:22, 23:30}; %{29:56, 5:25}; % {7:10, 18:20, 27:30};  % 1; %
